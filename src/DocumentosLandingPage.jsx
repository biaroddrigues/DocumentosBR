...
const docs = [
  ...
  { n: "07", name: "DECLARAÇÃO DE COMPARECIMENTO", text: "Para comprovar a presença do paciente em atendimento.", className: "doc-seven document-piece-text" },
  { n: "08", name: "ENCAMINHAMENTO FISIOTERAPÊUTICO", text: "Para formalizar o encaminhamento e a continuidade do cuidado.", className: "doc-eight document-piece-text" },
  { n: "09", name: "RELATÓRIO DE ALTA FISIOTERAPÊUTICA", text: "Para registrar a conclusão do acompanhamento fisioterapêutico.", className: "doc-nine document-piece-text" },
  { n: "10", name: "SOLICITAÇÃO DE EXAME COMPLEMENTAR", text: "Para solicitar exames complementares necessários à avaliação.", className: "doc-ten document-piece-text" },
  { n: "11", name: "CONSENTIMENTO PARA DRY NEEDLING", text: "Para registrar o consentimento informado antes do procedimento.", className: "doc-eleven document-piece-text" },
  { n: "12", name: "NOTIFICAÇÃO DE INTERRUPÇÃO DO ACOMPANHAMENTO", text: "Para formalizar a interrupção do acompanhamento fisioterapêutico.", className: "doc-twelve document-piece-text" },
];
...
export default function DocumentosLandingPage() {
  return <main>
    ...
    <section className="hero" id="top"><div className="hero-copy"><h1>A PARTE BUROCRÁTICA<br />DO SEU CONSULTÓRIO<br /><mark>NÃO PRECISA COMEÇAR</mark><br />DO ZERO.</h1><p>12 documentos profissionais, editáveis e reutilizáveis para sua rotina como fisioterapeuta.</p><div className="hero-offer"><strong>R$47</strong><span>PAGAMENTO<br />ÚNICO</span></div><BuyButton>QUERO MEUS DOCUMENTOS</BuyButton></div><HeroDocuments /><a className="scroll-cue" href="#demonstracao">VER COMO FUNCIONA <ArrowDown /></a></section>
    ...
    <DocMicroDemo />
    <section className="documents-section"><div className="section-heading"><h2>E NÃO É SÓ<br />O CONTRATO.</h2><p>DOZE DOCUMENTOS PARA MOMENTOS REAIS DA ROTINA.</p></div><div className="documents-editorial">{docs.map(doc => <article className={`document-piece ${doc.className}`} key={doc.name}>{doc.image && <div className="document-shot"><img src={doc.image} alt={`Página real: ${doc.name}`} loading="lazy" /></div>}<div className="document-caption"><span>{doc.n}</span><h3>{doc.name}</h3><p>{doc.text}</p></div></article>)}</div></section>
    ...
    <section className="main-offer"><div><h2>OS 12 DOCUMENTOS.<br /><mark>R$47 UMA VEZ.</mark></h2><ul><li>ACESSO VITALÍCIO</li><li>EDITÁVEIS E REUTILIZÁVEIS</li><li>PAGAMENTO ÚNICO</li><li>7 DIAS DE GARANTIA</li></ul></div><BuyButton>QUERO MEUS DOCUMENTOS</BuyButton></section>
